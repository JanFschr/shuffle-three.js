...
for (const side of [-1,1]) { addRail(-3.05,side*8.12,3,.3); addRail(3.05,side*8.12,3,.3); }
...
const goalGlow = (z,color) => { const m=new THREE.Mesh(new THREE.BoxGeometry(3.1,.06,.06),new THREE.MeshBasicMaterial({color}));m.position.set(0,.08,z);scene.add(m); }; goalGlow(-8.04,0x65ffdf); goalGlow(8.04,0xff376a);
...
function createBarrier(z,color){
  const group=new THREE.Group(),panel=new THREE.Mesh(new THREE.BoxGeometry(4.15,1.3,.07),glassMaterial(color));group.add(panel);
  const points=[];
  ...
  const shards=[];
  for(let i=0;i<16;i++){const shard=new THREE.Mesh(new THREE.TetrahedronGeometry(.13+Math.random()*.1),glassMaterial(color));shard.position.set(THREE.MathUtils.randFloatSpread(3.7),THREE.MathUtils.randFloatSpread(1),0);shard.visible=false;shard.userData.home=shard.position.clone();shard.userData.initialVelocity=new THREE.Vector3(THREE.MathUtils.randFloatSpread(2),Math.random()*2+.4,THREE.MathUtils.randFloatSpread(1));shard.userData.velocity=shard.userData.initialVelocity.clone();group.add(shard);shards.push(shard);}
...
function updateBarrier(barrier,hp,frame){barrier.cracks.material.opacity=Math.min(.9,(100-hp)/65);barrier.panel.material.opacity=.12+hp/100*.18;if(hp<=0)shatter(barrier);if(barrier.shattered)barrier.shards.forEach(shard=>{shard.userData.velocity.y-=3.2*frame;shard.position.addScaledVector(shard.userData.velocity,frame);shard.rotation.x+=frame*3;shard.rotation.z+=frame*2;});}
function processEvent(event){if(event.type==="rail"||event.type==="striker")audio.hit(event.type,event.intensity);if(event.type==="striker")state.rally++;if(event.type==="goal")audio.hit("goal",event.intensity);if(event.type==="win"){audio.hit("win",1);shatter(event.side==="player"?enemyBarrier:playerBarrier);ui.message.innerHTML=`<span class="eyebrow">VERTRAG BEENDET</span><h1>${event.side==="player"?"BARRIERE GEBROCHEN":"NACHT VORBEI"}</h1><p>${event.side==="player"?"OLD SAINT // BESIEGT":"DIE STADT VERGISST NICHTS"}</p><button id="restart">NOCH EIN DUELL <kbd>R</kbd></button>`;ui.message.classList.add("visible");document.querySelector("#restart")?.addEventListener("click",begin);}}
